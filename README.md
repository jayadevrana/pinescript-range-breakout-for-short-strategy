...
This repository contains a Pine Script source file saved as `pinescript_range_breakout_for_short_strategy.txt` so it opens easily on GitHub and can be copied into TradingView.
...
Range Breakout for short gives traders a practical chart workflow around RSI, breakout, trend following, support and resistance. The source is kept readable in a single `.txt` file so the logic can be reviewed, copied, and adapted inside TradingView.
...
3. Copy the code from `pinescript_range_breakout_for_short_strategy.txt`.
